...
const formPara = document.querySelector('.lorem-form');

console.log(formPara);

const numbPara = document.getElementById('amount');
console.log(numbPara);

const resultPara = document.querySelector('.lorem-text');
console.log(resultPara);

formPara.addEventListener('submit',function(e){
  // form prints the console and disappears.
  // To prevent this,
  e.preventDefault();

  // To convert typeOf(value) returns - String into number,we use parseInt
  const value = parseInt(numbPara.value);
  console.log(value);
  ...
  // If value < -1 or empty or  > 9 it should display atleast one paragraph.

  if(isNaN(value) || value <= 0 || value > 9){
  // resultPara.innerHTML = `<p class='result'> ${dummyText[randomNumber]} </p> `
  resultPara.textContent = `<p class='result'> ${dummyText[randomNumber]} </p> `
  }

  else{
  //slice() extracts a part of a string and returns the extracted part in a new string.
  // The method takes 2 parameters: the start position, and the end position (end not included).
    let userNumber = dummyText.slice(0,value);
    // console.log(userNumber);

    // Array.map():The map() method creates a new array by performing a function on each array element.
    userNumber = userNumber.map(function(para){
      return `<p class = 'result'> ${para} </p> <br/>`;
    }).join('$');
    console.log(userNumber);
    resultPara.innerHTML = userNumber;
  }
});
